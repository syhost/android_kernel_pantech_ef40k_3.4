...
int32_t msm_sensor_release(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	uint32_t delay = 0;
	CDBG("%s called\n", __func__);
#if 0//needtocheck #ifdef CONFIG_PANTECH_CAMERA
...
int32_t msm_sensor_config(struct msm_sensor_ctrl_t *s_ctrl, void __user *argp)
{
	...
	cdata.cfgtype);
		...
		case CFG_SET_WDR:
            if (s_ctrl->func_tbl->sensor_set_wdr) {
                rc = s_ctrl->func_tbl->sensor_set_wdr(s_ctrl, cdata.cfg.wdr);
            }	
            break;
        ...
        case CFG_SET_AF_CHECK:
            if (s_ctrl->func_tbl->sensor_check_af) {
                rc = s_ctrl->func_tbl->sensor_check_af(s_ctrl, cdata.cfg.focus.dir, cdata.cfg.focus.af_result); //p11634_add_af_check_130715
            }          
            break;    
        ...
        case CFG_SET_METERING_AREA:
            if (s_ctrl->func_tbl->sensor_set_metering_area) {
                rc = s_ctrl->func_tbl->sensor_set_metering_area(s_ctrl, cdata.cfg.focus_rect);
            }          
            break;    
...
#if 0//for_test//#ifndef CONFIG_PANTECH_CAMERA 
...
//enable_clk_failed: //for_test//#ifndef CONFIG_PANTECH_CAMERA 
#ifndef CONFIG_PANTECH_CAMERA    
