...
#endif
//Koine Kim C.G + 11019_ymlee, 2013-07-22
#ifdef CUSTOMER_HW_PANTECH
#define CMD_TXPOWER "TXPOWER"
#define CMD_CHANNEL		"CHANNEL"
#define CMD_PER		"FER/PER"
static uint prev_good_pkt = 0;
static uint prev_bad_pkt = 0;
#endif
...
//Pantech patch, Koine Kim C.G + 11019_ymlee
#ifdef CUSTOMER_HW_PANTECH
static void wl_android_reset_pktcnt(void)
{
	prev_good_pkt = 0;
	prev_bad_pkt = 0;
}

static int wl_android_get_per(struct net_device *net, char *command, int total_len)
{
	get_pktcnt_t pktcnt;
	...
	error = wldev_get_pktcnt(net, &pktcnt);
	if (error)
		return -1;

	dt_good_pkt = pktcnt.rx_good_pkt - prev_good_pkt;
	dt_bad_pkt = pktcnt.rx_bad_pkt - prev_bad_pkt;

	if ((dt_good_pkt + dt_bad_pkt) != 0)
		per = (dt_bad_pkt * 100) / (dt_good_pkt + dt_bad_pkt);
	else
		per = 0;

	prev_good_pkt = pktcnt.rx_good_pkt;
	prev_bad_pkt = pktcnt.rx_bad_pkt;

	bytes_written = snprintf(command, total_len, "%u", per);
	return bytes_written;
}
static int wl_android_get_channel(
	struct net_device *dev, char *command, int total_len)
{
	int bytes_written;
	struct channel_info ci;
	...
	if (error) {
	 ...
	 tmp_channel = ci.target_channel;
	DHD_INFO(("%s: tmp_channel=%d ci.target_channel=%d ci.scan_channel = %d\n", __FUNCTION__, tmp_channel,ci.target_channel,ci.scan_channel));
...
static int wl_android_get_txpower(struct net_device *net, char *command, int total_len)
{
  s32 txpwrdbm;
  u8 result;
  s32 err = 0;
  ...
  DHD_INFO(("wl_cfg80211_get_tx_power \n"));
  DHD_ERROR(("wl_cfg80211_get_tx_power \n"));

  err = wldev_iovar_getint(net, "qtxpower", &txpwrdbm);
  result = (u8) (txpwrdbm & ~WL_TXPWR_OVERRIDE);
  dbm = (s32) bcm_qdbm_to_mw(result);

  bytes_written = snprintf(command, total_len, "%d", dbm);

  DHD_INFO(("wl_cfg80211_get_tx_power (%d)\n", txpwrdbm));
  DHD_INFO(("wl_cfg80211_get_tx_power (%d)\n", result));
  DHD_INFO(("wl_cfg80211_get_tx_power (%d)\n", dbm));

  DHD_ERROR(("wl_cfg80211_get_tx_power (%d)\n", txpwrdbm));
  DHD_ERROR(("wl_cfg80211_get_tx_power (%d)\n", result));
  DHD_ERROR(("wl_cfg80211_get_tx_power (%d)\n", dbm));

  return bytes_written;
}
#endif /* CUSTOMER_HW_PANTECH */
...
#define PRIVATE_COMMAND_MAX_LEN	8192
...
#endif
#ifdef CUSTOMER_HW_PANTECH
	else if (strnicmp(command, CMD_PER, strlen(CMD_PER)) == 0) {
		bytes_written = wl_android_get_per(net, command, priv_cmd.total_len);
	}
	...
	else if (strnicmp(command, CMD_TXPOWER, strlen(CMD_TXPOWER)) == 0) {
		DHD_ERROR(("CMD_TXPOWER command command %s \n", command));
		DHD_ERROR(("CMD_TXPOWER command total_len %d \n", priv_cmd.total_len));    
		bytes_written = wl_android_get_txpower(net, command, priv_cmd.total_len);
		DHD_ERROR(("CMD_TXPOWER command %s \n", command));
		DHD_ERROR(("CMD_TXPOWER command %d \n", bytes_written));
	}
#endif /* CUSTOMER_HW_PANTECH */
#ifdef BCMCCX
