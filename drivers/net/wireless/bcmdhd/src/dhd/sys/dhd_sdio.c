...
#ifdef CONFIG_SKY_WLAN_BCM4330  // 2013-07-10 thkim_wifi add for distinguishing between semco and murata
#define MURATA_NV_PATH "/etc/wifi/nvram_murata_b2.txt"  
static void
dhdsdio_update_nvpath(uint8 *cis)
{
	int i = 0;
	int code, link;
	...
	do {
		...
		if ( code == 0x00 || code == 0xff )
              {
                     printk("%s: Found SEMCO ID: %s\n", __FUNCTION__,nv_path);
			break;
              }
		if ( code == 0x80 && link == 0x06 ) {
			uint8 *data = &cis[i];
			if ( *data == 0x81 && *(data+1) == 0x00 ) {
				strcpy(nv_path, MURATA_NV_PATH);
				printk("%s: Found MURATA ID: %s\n", __FUNCTION__,nv_path);
				break;
			}
		}

		i += link;
	} while ( i < SBSDIO_CIS_SIZE_LIMIT );
}
