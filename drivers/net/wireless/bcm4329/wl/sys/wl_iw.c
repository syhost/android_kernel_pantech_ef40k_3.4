 ...
 *
 * $Id: wl_iw.c,v 1.51.4.9.2.6.4.36.2.29.2.8 2010/11/30 00:22:29 Exp $
 */
...
)
{
	...
	if((error = dev_wlc_bufvar_set(dev, "keep_alive", (char*)&pkt, sizeof(wl_keep_alive_pkt_t))) < 0) {
		WL_ERROR(("%s: set keep_alive error\n", __FUNCTION__));
	}
...
)
{
	...
	char *p = extra;
	get_pktcnt_t pktcnt;
	static uint old_good_pkt=0, old_bad_pkt=0;
	uint dt_good_pkt, dt_bad_pkt;
	uint pkt_rate = 0;

	memset(&pktcnt, 0, sizeof(get_pktcnt_t));

	if(g_onoff == G_WLAN_SET_ON) {
		if((error = dev_wlc_ioctl(dev, WLC_GET_PKTCNTS, &pktcnt, sizeof(pktcnt))) < 0) {
			WL_ERROR(("%s: get packet_info error\n", __FUNCTION__));
			goto exit;
		}
	}

	dt_good_pkt = pktcnt.rx_good_pkt - old_good_pkt;
	dt_bad_pkt = pktcnt.rx_bad_pkt - old_bad_pkt;

	if((dt_good_pkt + dt_bad_pkt) != 0)
		pkt_rate = ((dt_bad_pkt * 100) / (dt_good_pkt + dt_bad_pkt));
	else
		pkt_rate = 0;

	old_good_pkt = pktcnt.rx_good_pkt;
	old_bad_pkt = pktcnt.rx_bad_pkt;

exit:
	p += snprintf(p, MAX_WX_STRING, "FER/PER %u", pkt_rate);
	wrqu->data.length = p - extra + 1;
...
)
{
	...
	error = dev_wlc_bufvar_get(dev, "wme_ac_sta", 
			(char*)acparam_cur, sizeof(acparam_cur));

	if ( error )
	{
		WL_ERROR(("%s: get wme_ac_sta error\n", __FUNCTION__));
		goto exit;
	}

	acp = (edcf_acparam_t *)buf;
	memcpy(acp, &acparam_cur[AC_VO], sizeof(edcf_acparam_t));
	acp->TXOP = acparam_cur[AC_VI].TXOP;

	error = dev_wlc_bufvar_set(dev, "wme_ac_sta", buf, WLC_IOCTL_SMLEN);
	if ( error )
	{
		WL_ERROR(("%s: set wme_ac_sta error\n", __FUNCTION__));
	}
...
#define WL_SCAN_PARAM_COUNT_MASK 0x0000ffff
...
)
{
    ...
    char buf[WLC_IOCTL_SMLEN];
    wl_bss_info_t *bi;
	uint16 assoc_ap_channel;
	int params_size = WL_SCAN_PARAMS_FIXED_SIZE + WL_NUMCHANNELS * sizeof(uint16);
	wl_scan_params_t *params;
	char *ptr;

	error = dev_wlc_ioctl(dev, WLC_GET_BSSID, &bssid, ETHER_ADDR_LEN); 
	if ( !error )
	{
		*(uint32*)buf = htod32(WLC_IOCTL_SMLEN);
		error = dev_wlc_ioctl(dev, WLC_GET_BSS_INFO, buf, WLC_IOCTL_SMLEN);
		if (error < 0)
		{
			WL_ERROR(("%s: get_bss_info error\n", __FUNCTION__));
			error = 1;
			goto exit;
		}
		else
		{
			...
			{
				assoc_ap_channel = CHSPEC_CHANNEL(dtohchanspec(bi->chanspec));

				params = (wl_scan_params_t*)kmalloc(params_size, GFP_KERNEL);

				if (params == NULL)
				{
					WL_ERROR(("%s: Error allocating %d bytes for scan params\n", __FUNCTION__, params_size));
					error = 1;
					goto exit;
				}
				...
				params->channel_list[0] = assoc_ap_channel;

				ptr = (char*)params->channel_list + params->channel_num * sizeof(uint16);
				params_size = ptr - (char*)params;

				error =  dev_wlc_ioctl(dev, WLC_SCAN, params, params_size);

				/*
				printk("nprobes = %d\n", params->nprobes);
        		printk("active_time = %d\n", params->active_time);
        		printk("passive_time = %d\n", params->passive_time);
        		printk("home_time = %d\n", params->home_time);
				printk("channel_list = %d\n", params->channel_list[0]);
				printk("ssid_len = %d\n", params->ssid.SSID_len);
				printk("ssid = %s\n", params->ssid.SSID);
				printk("channel_num = %d\n", params->channel_num);
				*/
...
wl_iw_ch_to_chanspec(int ch, wl_join_params_t *join_params, int *join_params_size)
{
	...
	if (ch != 0) {
		...
		join_params->params.chanspec_list.num = 1;
		join_params->params.chanspec_list.chanspec[0] = ch;

		if (join_params->params.chanspec_list.chanspec[0])
		...
		*join_params_size += WL_ASSOC_PARAMS_FIXED_SIZE +
			join_params->params.chanspec_list.num * sizeof(chanspec_t);

		
		join_params->params.chanspec_list.chanspec[0]  &= WL_CHANSPEC_CHAN_MASK;
		join_params->params.chanspec_list.chanspec[0] |= chanspec;
		join_params->params.chanspec_list.chanspec[0] =
		        htodchanspec(join_params->params.chanspec_list.chanspec[0]);

		join_params->params.chanspec_list.num =
			htod32(join_params->params.chanspec_list.num);

		WL_TRACE(("%s  join_params->params.chanspec_list.chanspec[0]= %X\n",
			__FUNCTION__, join_params->params.chanspec_list.chanspec[0]));
	}
	return 1;
}
...
)
{
	...
	if((error = dev_wlc_ioctl(dev, WLC_CURRENT_PWR, (char *)&power, sizeof(tx_power_t))))
	{
		WL_ERROR(("%s: get_current_power error\n", __FUNCTION__));
		vwrq->value = 0;
	}
	else
	{
		vwrq->value = DIV_QUO(power.est_Pout[0], 4);
	}
...
#if 1 //lee.eunsuk 20130603 build error ,array subscript is above array bounds
	...
	get_channel_retry:
			...
			if (ret < 0 || dtoh32(chosen) == 0) {
				if (retry++ < 3)
					...
					WL_ERROR(("can't get auto channel sel, "
						"err = %d, chosen = %d\n",
						ret, chosen));
					goto fail;
				}
...
#ifdef CUSTOMER_HW_PT
                if(!strncmp(token, "SSID=", strlen("SSID="))) {
                        char *ptr = *str_ptr;
                        while ((ptr = strstr(ptr, ",SEC="))) {
                                *str_ptr = ptr++;
                        }
                }
#endif
	...
	char *ext)
{
	...
	WL_TRACE((">Got FW_RELOAD cmd:"
				"info->cmd:%x, info->flags:%x, u.data:%p, u.len:%d, fw_path:%p, "
				"len:%d\n",
				info->cmd, info->flags,
...
#ifdef CUSTOMER_HW_PT
	    else if (strnicmp(extra, "KEEPALIVE", strlen("KEEPALIVE")) == 0)
			ret = wl_iw_set_keep_alive(dev, info, (union iwreq_data *)dwrq, extra);
	    else if (strnicmp(extra, "FER/PER", strlen("FER/PER")) == 0)
			ret = wl_iw_get_fer_per(dev, info, (union iwreq_data *)dwrq, extra);
	    else if (strnicmp(extra, "WMECERTMODE", strlen("WMECERTMODE")) == 0)
			ret = wl_iw_set_wmecertmode(dev, info, (union iwreq_data *)dwrq, extra);
		else if (strnicmp(extra, "ANRITSU_TISTRP", strlen("ANRITSU_TISTRP")) == 0)
			ret = wl_iw_set_chanscan(dev, info, (union iwreq_data *)dwrq, extra);
#endif
