...
dhd_prot_ioctl(dhd_pub_t *dhd, int ifidx, wl_ioctl_t * ioc, void * buf, int len)
{
	...
	if ((!ret) && (ioc->cmd == WLC_SET_VAR) && (!strcmp(buf, "wme_dp"))) {
        ...
        else if ((!ret) && (ioc->cmd == WLC_SET_VAR)
                        && (!strcmp(buf, "keep_alive"))) {
                wl_keep_alive_pkt_t *ppkt;
                char *p = (char *)buf;
                p += strlen("keep_alive") + 1;
                ppkt = (wl_keep_alive_pkt_t *)p;
                dhd->keep_alive.period_msec = ppkt->period_msec;
        }
#endif
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	uint roamvar = 1;
#endif
#ifdef CONFIG_BCM4329_HT_WEP_ENABLE
	uint ht_wep = 1;
#endif
...
#define htod32(i) i
	...
	if (!ret) {
		bcm_mkiovar("cur_etheraddr", (void *)&ea_addr, ETHER_ADDR_LEN, iovbuf, sizeof(iovbuf));
	...
	if (roamvar == 0)
        {
                ...
                int roam_scan_period = 10; // 30; /* in sec */
                int roam_fullscan_period = 120; /* in sec */
                int roam_trigger = -75; //-85;
                int roam_delta = 15;
                int band;
                ...
                if (dhd_wl_ioctl_cmd(dhd, WLC_GET_BAND, \
                                (char *)&band, sizeof(band), FALSE, 0) < 0)
...
#ifdef CUSTOMER_HW_PT
        /* Set keep_alive */
        if(dhd->keep_alive.period_msec > 0) {
                wl_keep_alive_pkt_t pkt;

                pkt.period_msec = dhd->keep_alive.period_msec;
                pkt.len_bytes = 0;

		buf_len = bcm_mkiovar("keep_alive",
				(char*)&pkt, sizeof(wl_keep_alive_pkt_t), 
				buf, sizeof(buf));

                if (dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, buf, buf_len, TRUE, 0) < 0)
                {
                        DHD_ERROR(("%s:can't set keep alive\n", __FUNCTION__));
                }
        }
#endif
...
#ifdef CONFIG_BCM4329_HT_WEP_ENABLE
		/* set allow_ht_wep */
		bcm_mkiovar("allow_ht_wep", (char *)&ht_wep, 4, iovbuf, sizeof(iovbuf));
		dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0);
#endif
