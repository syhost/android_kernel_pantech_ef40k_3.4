 ...
 *
 * $Id: dhd_common.c,v 1.5.6.8.2.6.6.30.2.31.2.20 2010/11/29 21:00:32 Exp $
 */
#include <typedefs.h>
#include <osl.h>
				...
				if (ifevent->ifidx > 0 && ifevent->ifidx < DHD_MAX_IFS) {
					if (ifevent->action == WLC_E_IF_ADD) {
						...
						if (dhd_use_p2p) {
							...
							snprintf(p2piface_name, IFNAMSIZ, "%s%d", P2P_IFNAME, ifevent->ifidx);
