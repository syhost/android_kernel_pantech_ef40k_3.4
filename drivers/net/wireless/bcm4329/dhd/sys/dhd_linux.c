...
#ifdef CUSTOM_PANTECH //lee.eunsuk , build error: implicit declaration of function 'init_MUTEX'
...
static int dhd_sleep_pm_callback(struct notifier_block *nfb, unsigned long action, void *ignored)
{
	...
	switch (action)	{
		...
		case PM_SUSPEND_PREPARE:
			...
			printf("%s: dhd_mmc_suspend = TRUE\n", __FUNCTION__);
		ret = NOTIFY_OK;
		break;
		case PM_POST_HIBERNATION:
		case PM_POST_SUSPEND:
			dhd_mmc_suspend = FALSE;
			printf("%s: dhd_mmc_suspend = FALSE\n", __FUNCTION__);
#ifdef CUSTOMER_HW_PT
			if ( _dhd )
...
#ifndef CUSTOM_PANTECH //lee.eunsuk 20130604 build error
static void
...
#ifndef CUSTOM_PANTECH //build error	
