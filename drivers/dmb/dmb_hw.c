...
{
  int rc =0;
  DMB_MSG_HW("[%s] Configuring MSM_XO  %d \n", __func__, on);

  if(!xo_handle_a1)
  {
#if (defined(CONFIG_MACH_MSM8X60_EF35L) || defined(CONFIG_MACH_MSM8X60_EF39S) || defined(CONFIG_MACH_MSM8X60_EF40S) || defined(CONFIG_MACH_MSM8X60_EF40K))
    xo_handle_a1 = msm_xo_get(MSM_XO_TCXO_A1, "DMB_XTAL");
#elif defined(CONFIG_MACH_MSM8X60_EF65L)
    xo_handle_a1 = msm_xo_get(MSM_XO_TCXO_D0, "DMB_XTAL");
#endif
  }
