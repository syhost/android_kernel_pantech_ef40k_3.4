...
void t3700_ch_scan_start(int freq, int band, unsigned char for_air)
{
  ...
  if(((dmb_mode == TDMB_MODE_AUTOSCAN) && (freq == 175280)) || dmb_mode == TDMB_MODE_NETBER
#ifdef FEATURE_DMB_AUTOSCAN_DISCRETE
      || ((dmb_mode == TDMB_MODE_SCAN_METRO) && (freq == 181280))
#endif
  )
  {
    msleep(100);
...
{ 
    ...
    stMultiCh->nSetCnt = g_stEnsembleInfo.nSetCnt;
    
    pstChInfo  = &stMultiCh->astSubChInfo[0];
    
    pstChInfo->ulRFFreq         = g_stEnsembleInfo.astSubChInfo[0].ulRFNum;
    pstChInfo->uiEnsembleID     = g_stEnsembleInfo.astSubChInfo[0].uiEnsumbleID;
    pstChInfo->ucSubChID        = g_stEnsembleInfo.astSubChInfo[0].uiSubChID;
    pstChInfo->uiStarAddr       = g_stEnsembleInfo.astSubChInfo[0].uiStarAddr;
    pstChInfo->uiBitRate        = g_stEnsembleInfo.astSubChInfo[0].uiBitRate;
    pstChInfo->uiTmID           = g_stEnsembleInfo.astSubChInfo[0].uiTmID;
    pstChInfo->ucSlFlag         = g_stEnsembleInfo.astSubChInfo[0].uiSlFlag;
    pstChInfo->ucTableIndex     = g_stEnsembleInfo.astSubChInfo[0].ucTableIndex;
    pstChInfo->ucOption         = g_stEnsembleInfo.astSubChInfo[0].ucOption;
    pstChInfo->ucProtectionLevel= g_stEnsembleInfo.astSubChInfo[0].uiProtectionLevel;
    pstChInfo->uiDifferentRate  = g_stEnsembleInfo.astSubChInfo[0].uiDifferentRate;
    pstChInfo->uiSchSize        = g_stEnsembleInfo.astSubChInfo[0].uiSchSize;
  
    if(dmb_mode == TDMB_MODE_NETBER)
    {
#if (defined(INC_MULTI_CHANNEL_FIC_UPLOAD) && defined(INC_MULTI_CHANNEL_NETBER_NOT_FIC_UPLOAD))
      pstChInfo->ucServiceType = g_stEnsembleInfo.astSubChInfo[0].uiServiceType = TDMB_BB_SVC_NETBER;
#else
    
#ifdef FEATURE_NETBER_TEST_ON_AIR
      pstChInfo->ucServiceType = g_stEnsembleInfo.astSubChInfo[0].uiServiceType = TDMB_BB_SVC_NETBER;
#else
      pstChInfo->ucServiceType = g_stEnsembleInfo.astSubChInfo[0].uiServiceType;
#endif /* FEATURE_NETBER_TEST_ON_AIR */
    ...
    {
      pstChInfo->ucServiceType = (TDMB_BB_SVC_DMB == g_stEnsembleInfo.astSubChInfo[0].uiServiceType) ? 0x18 : 0;
    }
  
#ifdef INC_MULTI_CHANNEL_ENABLE
    INC_MULTI_SORT_INIT();
    result = INC_MULTI_START_CTRL(TMDB_I2C_CTRL_ID, stMultiCh);
#else
    result = INC_CHANNEL_START(TMDB_I2C_CTRL_ID, stMultiCh);
#endif
...
#if defined(FEATURE_TEST_ON_BOOT) && !defined(INC_MULTI_CHANNEL_ENABLE)
    dmb_data_dump(msc_buffer, INC_INTERRUPT_SIZE, FILENAME_RAW_MSC);

    for(uiLoop=0; uiLoop<(INC_INTERRUPT_SIZE); uiLoop+=188)
    {
      if(msc_buffer[uiLoop+0] != 0x47)
...
/*====================================================================
FUNCTION       t3700_if_test
DESCRIPTION 
DEPENDENCIES
RETURN VALUE
SIDE EFFECTS
======================================================================*/
void t3700_if_test(void)
{
