...
INC_UINT8 INC_INIT(INC_UINT8 ucI2CID)
{
  ...
  INC_SPI_INIT();
  TDMB_MSG_INC_BB("SPI MPI BASE(%x)(6) Read [%x]\n",APB_MPI_BASE, INC_CMD_READ(ucI2CID, APB_MPI_BASE));
#endif
...
INC_UINT8 INC_SYNCDETECTOR(INC_UINT8 ucI2CID, INC_UINT32 ulFreq, INC_UINT8 ucScanMode)
{
  ...
  while(1)
  {
    ...
    if(!wIsNullSync)
    {
      ...
      {
        TDMB_MSG_INC_BB("[%s] ERROR_SYNC_NO_SIGNAL1\n", __func__);
      ...
      else
      {
        if(awData[1] <= 0x2000 && awData[3] == 0xf)
        {
          ...
          TDMB_MSG_INC_BB("[%s] ERROR_SYNC_NO_SIGNAL2\n", __func__);
          pInfo->nBbpStatus = ERROR_SYNC_NO_SIGNAL;
          break;
        }
        else
...
#else
    if(INC_STOP(ucI2CID) != INC_SUCCESS) TDMB_MSG_INC_BB("INC_STOP Error %x\n", pInfo->nBbpStatus);
    if(INC_READY(ucI2CID, pChInfo[0]->ulRFFreq) != INC_SUCCESS) TDMB_MSG_INC_BB("INC_READY Error %x\n", pInfo->nBbpStatus);
    if(INC_SYNCDETECTOR(ucI2CID, pChInfo[0]->ulRFFreq, 0) != INC_SUCCESS) TDMB_MSG_INC_BB("INC_SYNCDETECTOR Error %x\n", pInfo->nBbpStatus);
#endif
