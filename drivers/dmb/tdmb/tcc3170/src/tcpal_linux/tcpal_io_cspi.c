...
static struct spi_device *tcpal_find_cspi_device(void)
{
	...
	if (pdev)
		tcbd_debug(DEBUG_TCPAL_CSPI, "spi_device :%p\n", spi_device);
	put_device(&spi_master->dev);
	return spi_device;
}
...
static s32 tcpal_cspi_open(void)
{
...
#ifdef FEATURE_DMB_SPI_IF
	spi_data->spi_dev = dmb_spi_setup();
	if(spi_data->spi_dev == NULL)
	{
		tcbd_debug(DEBUG_ERROR, "Setup Fail !! \n");
		return -1;
	}
	else
	{
		tcbd_debug(DEBUG_ERROR, "Setup OK ~ mode[%d] Hz[%d]\n",spi_data->spi_dev->mode, spi_data->spi_dev->max_speed_hz);
		return 0;
	}
#endif
	...
	if (spi_data->spi_dev) {
		...
		if (ret < 0) {
			tcbd_debug(DEBUG_ERROR, "spi_setup failed :%d\n", ret);
			goto cspi_init_fail;
		}
	}
#endif /*__USE_TC_CPU__*/
