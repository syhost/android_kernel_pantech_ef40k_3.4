...
#elif defined(FEATURE_DMB_SPI_IF)
  tcbd_init_parser(0, (tcbd_stream_callback)tcpal_irq_stream_callback);
#endif /* FEATURE_DMB_SPI_IF */
...
int tcc3170_subch_start(uint8 *regs, uint32 data_rate)
{
  ...
  //for TDMB new scenario
  tcc3170_ch_scan_start(dev_data->tcc_freq, 0,0);
...
int tcc3170_read_fib(uint8 *fibs)
{
  ...
  TDMB_MSG_TCC_BB("[%s] size[%d] [%x]\n", __func__, wFicLen, fibs[0]);
  ...
  dmb_data_dump(fibs, wFicLen, FILENAME_RAW_FIC);
#endif
...
void tcc3170_set_subchannel_info(void * sub_ch_info)
{
  ...
  dev_data->tcc_freq = tcc_ch_info->ulRFNum;
}
...
int tcc3170_Ant_Level(uint32 pcber,int rssi)
{
  ...
  uint16 hystery_value[]= {50, 100, 100, 100, 50};
