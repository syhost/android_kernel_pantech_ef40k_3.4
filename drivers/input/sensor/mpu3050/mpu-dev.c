...
// P12095 110317 must be locked 'mpu config' when msm communicate with mpu3050 via i2c by ioctl
#define FEATURE_IOCTL_MUTEX_ENABLE	
...
#else
#define MSM_GSBI5_QUP_I2C_BUS_ID 2 //#define MSM_GSBI9_QUP_I2C_BUS_ID 2
#endif
#endif
static struct mpu3050_platform_data mpu3050_data = {
    ...
    .accel = {
        ...
        .adapt_num = MSM_GSBI5_QUP_I2C_BUS_ID, //MSM_GSBI5_QUP_I2C_BUS_ID, // The i2c bus to which the mpu device is
        // connected
...
#elif defined(CONFIG_SENSORS_BMA250)
    .accel = {
...
#ifdef FEATURE_IOCTL_MUTEX_ENABLE
static struct mutex mpu_ioctl_lock;
#endif
				      ...
				      unsigned long arg)
{
	...
	while (i-- > 0) {
		...
		if(rdwr_pa[i].buf != NULL) kfree(rdwr_pa[i].buf);
	}
	if(data_ptrs != NULL) kfree(data_ptrs);
	if(rdwr_pa != NULL) kfree(rdwr_pa);
	return res;
}
...
//	if( (cmd!=0x707) && (cmd!=0x46) ) printk("mpu-dev.ioctl 0x%04X\n", cmd);

#ifdef FEATURE_IOCTL_MUTEX_ENABLE
	mutex_lock(&mpu_ioctl_lock);
#endif
...
#ifdef FEATURE_IOCTL_MUTEX_ENABLE //p12911 2011-12-19 modify here 
	pressure_adapter = NULL;
#else
	pressure_adapter =
	...
	case MPU_READ_COMPASS:
		{
			// P12095 110721 for geomatnetic calibration
			//unsigned char data[6];
...
#if 0//defined(CONFIG_MACH_MSM8X60_EF33S) || defined(CONFIG_MACH_MSM8X60_EF34K) || defined(CONFIG_MACH_MSM8X60_EF35L)
#define GPIO_GYRO_INT 30
#define COMPASS_RESET 62

static int init_gyro_hw_setting(void)
{
	// reset compass
	gpio_tlmm_config(GPIO_CFG(COMPASS_RESET, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	gpio_set_value(COMPASS_RESET, 1);

	// init gyro's interrupt pin ( was don't care because of non-use )
	gpio_tlmm_config(GPIO_CFG(GPIO_GYRO_INT, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
