...
int snd_extamp_write_all_reg(extamp_info_t tCurrentExtampInfo)
{
	...
	if(write_status!=0)  {
		AMP_ERR( "[SKY Audio] snd_extamp_write_all_reg (Write Status:%x)\n", write_status);
		return 1;
	}
	else {
		AMP_ERR( "[SKY Audio] snd_extamp_write_all_reg (No change)  \n");
		return 0;
	}
}
...
void snd_extamp_api_SetDevice(int on, uint32_t cad_device)
{
	...
	switch(cad_device)
	{
		...
		if (on) 
		{
	          ...
	          tCurrentExtampInfo.hpl_ctl_reg_val       = ZCD_DIS | SLEW_DIS | HPLM_UNMUTE | HPLVOL_M_10_DB;
	          tCurrentExtampInfo.hpr_ctl_reg_val       = HPGAIN_P_3_DB | HPRM_UNMUTE | HPRVOL_M_10_DB;
		...
		if (on)
		{
	          if(curMode==MODE_IN_CALL)      
	              /* 20111018 frogLove - SP PGAINB volume change (0 -> 3dB) */
	              tCurrentExtampInfo.inp_gain_ctl_reg_val = INADIFF_STEREO_SINGLE | INBDIFF_DIFFERENTIAL | PGAINA_P_0_DB | PGAINB_P_3_DB;
	          else
	              /* 20111012 frogLove - SP PGAINB volume change (3 -> 0 -> 3dB) */
	              tCurrentExtampInfo.inp_gain_ctl_reg_val = INADIFF_STEREO_SINGLE | INBDIFF_DIFFERENTIAL | PGAINA_P_0_DB | PGAINB_P_3_DB;

	          tCurrentExtampInfo.sp_mixer_ctl_reg_val = SPKMIX_INB2;

	          if(curMode==MODE_IN_CALL)      
	              /* 20111012 frogLove - SP OUT volume change (-4 -> -2dB) */
	              tCurrentExtampInfo.sp_ctl_reg_val = FFM_FF_MODE | SPKM_UNMUTE | SPKVOL_M_2_DB;
	          else
	              /* 20111012 frogLove - SP OUT volume change (-4 -> 6 -> -4dB) */
